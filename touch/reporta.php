...
<center>
        ...
        <div style="max-width: 1000px; margin-top: 4px;">
          <div class="card">
            <div class="card-header">
              ...
              <h2 class="card-title"><a href="reportpagea.php"><button class="btn btn-info">Back</button></a> <b>Recently added goods reports for today  <span id="span1"><script type="text/javascript">
                var dat = new Date();
    ...
    //$produce = $_POST['produce'];
     // formating the dates
      ...
      $date2 = $date->format('Y-m-d');
          ...
          $sql ="SELECT category, type, quantity, sum(price) price, transaction_date,Username, Sum(quantity*price) as value
            FROM perfecttouch.product2
            WHERE transaction_date  BETWEEN '$dat1' AND '$dat2' GROUP BY type";
  ...
  $sql ="SELECT category, type,count(type) as typeno  , sum(quantity) as qinstock,
            ...
            FROM perfecttouch.product2 WHERE transaction_date  BETWEEN '$dat1' AND '$dat2'";
        $result = $connect->query($sql);
        if ($result-> num_rows >0) {
          while ($row = $result-> fetch_assoc()) {
