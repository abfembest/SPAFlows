...
<html lang="en">
 ...
 $sql = "SELECT invoice_id,transaction_date FROM perfecttouch.sales WHERE status = 0 AND username = '$name' LIMIT 1";
...
<?php
  ...
  ?>
            ...
            <table style="text-align: center;">
                    ...
                    <?php
                    include 'db_connect.php';
 $sql = "SELECT invoice_id, fullname, type, quantity, unit_price, (unit_price) AS selling_price, transaction_date  FROM perfecttouch.sales  WHERE status = 0 AND username = '$name'";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {?>
      ...
      <tr>
          ...
          <td class="description"><?php echo $row['selling_price']; ?></td>
             ...
             <?php                 
              ...
              ?>
             <?php
            $sql = "SELECT quantity, fullname, unit_price, SUM(unit_price) AS Amount, count(quantity)AS total_quantity FROM perfecttouch.sales WHERE status = 0 AND username = '$name'";
            $result = $connect->query($sql);
            if ($result-> num_rows >0) {
              while ($row = $result-> fetch_assoc()) {
        ...
        <a href="../servicesa.php"><button id="back" class="hidden-print"><i class="fa fa-left">Back</a></button>
