...
<html lang="en">
    ...
    <body style=" font-size: 15px;text-align: center; font-style: bold;"><b>
        ...
        <div class="ticket">
          <div style="background-color: pink;" class="hidden-color">
            ...
            <p class="centered"><I>OLAOLUWA VENTURES</I>
                <br>AGIP Area Ede, Osun State, Nigeria.
 ...
 $sql = "SELECT invoice_id,transaction_date FROM olaoluwa.sales WHERE invoice_id = '$invoiceid' LIMIT 1";
...
<?php
  ...
  ?>
            ...
            <table style="text-align: center;">
                    ...
                    <?php
                    include 'db_connect.php';
 $sql = "SELECT invoice_id, type, quantity, unit_price, (quantity *unit_price) AS selling_price, transaction_date FROM olaoluwa.sales WHERE invoice_id = '$invoiceid'";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {?>
      ...
      <tr>
          ...
          <td class="description"><?php echo $row['selling_price']; ?></td>
             ...
             <?php 
             include 'db_connect.php';
            $sql = "SELECT quantity, unit_price, SUM(quantity*unit_price) AS Amount, SUM(quantity)AS total_quantity FROM olaoluwa.sales WHERE invoice_id = '$invoiceid'";
            $result = $connect->query($sql);
            if ($result-> num_rows >0) {
              while ($row = $result-> fetch_assoc()) {?>
